...
} from "./chunk-6JI7VB2V.js";
import {
  useThemeProps
} from "./chunk-YWLLQTMT.js";
import {
  require_react_dom
...
import {
  ...
  useRtl,
  useTimeout
} from "./chunk-JBHBSE5V.js";
import {
  _extends
} from "./chunk-ZIE44QAC.js";
import {
  require_jsx_runtime
...
var require_use_sync_external_store_shim_development = __commonJS({
  "node_modules/use-sync-external-store/cjs/use-sync-external-store-shim.development.js"(exports) {
    ...
    (function() {
      ...
      var React155 = require_react(), objectIs = "function" === typeof Object.is ? Object.is : is2, useState26 = React155.useState, useEffect44 = React155.useEffect, useLayoutEffect8 = React155.useLayoutEffect, useDebugValue = React155.useDebugValue, didWarnOld18Alpha = false, didWarnUncachedGetSnapshot = false, shim = "undefined" === typeof window || "undefined" === typeof window.document || "undefined" === typeof window.document.createElement ? useSyncExternalStore$1 : useSyncExternalStore$2;
...
var useGridVirtualScroller = () => {
  ...
  const isRenderContextReady = React35.useRef(false);
  ...
  const previousSize = React35.useRef(null);
  const mainRefCallback = React35.useCallback((node) => {
  ...
  const scrollPosition = React35.useRef(((_a = rootProps.initialState) == null ? void 0 : _a.scroll) ?? EMPTY_SCROLL_POSITION);
  const ignoreNextScrollEvent = React35.useRef(false);
  ...
  const onContentSizeApplied = React35.useCallback((node) => {
    if (!node) {
      return;
    }
...
function useProps(allProps) {
  return React49.useMemo(() => groupForwardedProps(allProps), [allProps]);
}
...
var GridRoot = forwardRef2(function GridRoot2(props, ref) {
  ...
  const rootMountCallback = React55.useCallback((node) => {
    if (node === null) {
      return;
    }
...
var ColumnHeaderMenuIcon = React59.memo((props) => {
  var _a, _b;
  const {
    colDef,
    open,
  ...
  const handleMenuIconClick = React59.useCallback((event) => {
    event.preventDefault();
    event.stopPropagation();
...
function GridColumnHeaderTitle(props) {
  ...
  const rootProps = useGridRootProps();
  const titleRef = React61.useRef(null);
  const [tooltip, setTooltip] = React61.useState("");
  const handleMouseOver = React61.useCallback(() => {
...
function GridColumnHeaderSeparatorRaw(props) {
  ...
  const stopClick = React62.useCallback((event) => {
    event.preventDefault();
    event.stopPropagation();
  }, []);
  return (
...
var GridColumnHeaderSeparator = React62.memo(GridColumnHeaderSeparatorRaw);
...
function GridColumnHeaderItem(props) {
  ...
  const rootProps = useGridRootProps();
  const isRtl = useRtl();
  const headerCellRef = React64.useRef(null);
  const columnMenuId = useId();
  const columnMenuButtonId = useId();
  const iconButtonRef = React64.useRef(null);
  const [showColumnMenuIcon, setShowColumnMenuIcon] = React64.useState(columnMenuOpen);
  const isDraggable = React64.useMemo(() => !rootProps.disableColumnReorder && !disableReorder && !colDef.disableReorder, [rootProps.disableColumnReorder, disableReorder, colDef.disableReorder]);
  let headerComponent;
  ...
  const publish = React64.useCallback((eventName) => (event) => {
    if (isEventTargetInPortal(event)) {
      return;
    }
    apiRef.current.publishEvent(eventName, apiRef.current.getColumnHeaderParams(colDef.field), event);
  }, [apiRef, colDef.field]);
  const mouseEventsHandlers = React64.useMemo(() => ({
  ...
  const draggableEventHandlers = React64.useMemo(() => isDraggable ? {
    onDragStart: publish("columnHeaderDragStart"),
    onDragEnter: publish("columnHeaderDragEnter"),
    onDragOver: publish("columnHeaderDragOver"),
    onDragEnd: publish("columnHeaderDragEnd")
  } : {}, [isDraggable, publish]);
  const columnHeaderSeparatorProps = React64.useMemo(() => ({
  ...
  const columnTitleIconButtons = (0, import_jsx_runtime44.jsxs)(React64.Fragment, {
  ...
  const style = React64.useMemo(() => attachPinnedStyle(_extends({}, props.style), isRtl, pinnedPosition, pinnedOffset), [pinnedPosition, pinnedOffset, props.style, isRtl]);
...
var GridColumnHeaderSortIcon = React66.memo(GridColumnHeaderSortIconRaw);
...
function GridColumnHeaderFilterIconButton(props) {
  ...
  const panelId = useId();
  const toggleFilter = React67.useCallback((event) => {
    event.preventDefault();
    event.stopPropagation();
    const {
      open,
...
var GridColumnMenuContainer = forwardRef2(function GridColumnMenuContainer2(props, ref) {
  ...
  const handleListKeyDown = React69.useCallback((event) => {
    if (event.key === "Tab") {
      event.preventDefault();
    }
...
var useGridColumnMenuSlots = (props) => {
  ...
  const processedComponents = React70.useMemo(() => _extends({}, defaultSlots2, slots), [defaultSlots2, slots]);
  const processedSlotProps = React70.useMemo(() => {
  ...
  const userItems = React70.useMemo(() => {
...
function GridColumnMenuHideItem(props) {
  ...
  const toggleColumn = React71.useCallback((event) => {
    if (disabled) {
      return;
    }
...
function GridColumnMenuManageItem(props) {
  ...
  const apiRef = useGridApiContext();
  const rootProps = useGridRootProps();
  const showColumns = React72.useCallback((event) => {
    onClick(event);
...
function GridColumnMenuFilterItem(props) {
  ...
  const apiRef = useGridApiContext();
  const rootProps = useGridRootProps();
  const showFilter = React74.useCallback((event) => {
    onClick(event);
...
function GridColumnMenuSortItem(props) {
  ...
  const rootProps = useGridRootProps();
  const sortDirection = React75.useMemo(() => {
    if (!colDef) {
      return null;
    }
  ...
  const sortingOrder = colDef.sortingOrder ?? rootProps.sortingOrder;
  const onSortMenuItemClick = React75.useCallback((event) => {
    onClick(event);
  ...
  return (0, import_jsx_runtime54.jsxs)(React75.Fragment, {
...
var GridPanel = forwardRef2((props, ref) => {
  ...
  const [isPlaced, setIsPlaced] = React79.useState(false);
  const handleClickAway = React79.useCallback(() => {
    apiRef.current.hidePreferences();
  }, [apiRef]);
  const handleKeyDown = React79.useCallback((event) => {
    if (event.key === "Escape") {
      apiRef.current.hidePreferences();
    }
  }, [apiRef]);
  const modifiers = React79.useMemo(() => [{
    name: "flip",
    enabled: true,
    options: {
  ...
  const [anchorEl, setAnchorEl] = React79.useState(null);
  React79.useEffect(() => {
    var _a, _b;
...
var GridFilterForm = forwardRef2(function GridFilterForm2(props, ref) {
  ...
  const filterSelectorRef = React83.useRef(null);
  ...
  const sortedFilteredColumns = React83.useMemo(() => {
    switch (columnsSort) {
      case "asc":
  ...
  const currentOperator = React83.useMemo(() => {
    var _a2;
  ...
  const changeColumn = React83.useCallback((event) => {
  ...
  const changeOperator = React83.useCallback((event) => {
  ...
  const changeLogicOperator = React83.useCallback((event) => {
  ...
  React83.useImperativeHandle(focusElementRef, () => ({
    focus: () => {
      var _a2;
...
var GridFilterPanel = forwardRef2(function GridFilterPanel2(props, ref) {
  ...
  const lastFilterRef = React84.useRef(null);
  const placeholderFilter = React84.useRef(null);
  const {
    logicOperators = [GridLogicOperator.And, GridLogicOperator.Or],
    columnsSort,
    filterFormProps,
  ...
  const applyFilterLogicOperator = React84.useCallback((operator) => {
    apiRef.current.setFilterLogicOperator(operator);
  }, [apiRef]);
  const getDefaultFilter = React84.useCallback(() => {
  ...
  const getNewFilter = React84.useCallback(() => {
  ...
  const {
    readOnlyFilters,
    validFilters
  } = React84.useMemo(() => items.reduce((acc, item) => {
  ...
  const addNewFilter = React84.useCallback(() => {
  ...
  const deleteFilter = React84.useCallback((item) => {
  ...
  const handleRemoveAll = React84.useCallback(() => {
...
function GridColumnsManagement(props) {
  ...
  const isResetDisabled = React85.useMemo(() => checkColumnVisibilityModelsSame(columnVisibilityModel, initialColumnVisibilityModel), [columnVisibilityModel, initialColumnVisibilityModel]);
  ...
  const hideableColumns = React85.useMemo(() => currentColumns.filter((col) => col.hideable), [currentColumns]);
  const allHideableColumnsVisible = React85.useMemo(() => hideableColumns.every((column) => columnVisibilityModel[column.field] == null || columnVisibilityModel[column.field] !== false), [columnVisibilityModel, hideableColumns]);
  const allHideableColumnsHidden = React85.useMemo(() => hideableColumns.every((column) => columnVisibilityModel[column.field] === false), [columnVisibilityModel, hideableColumns]);
...
var GridToolbarDensitySelector = forwardRef2(function GridToolbarDensitySelector2(props, ref) {
  ...
  const startIcon = React87.useMemo(() => {
    switch (density) {
      case "compact":
...
var GridToolbarFilterButton = forwardRef2(function GridToolbarFilterButton2(props, ref) {
  ...
  const tooltipContentNode = React88.useMemo(() => {
    if (preferencePanel.open) {
...
var GridToolbarExport = forwardRef2(function GridToolbarExport2(props, ref) {
  ...
  return (0, import_jsx_runtime69.jsx)(GridToolbarExportContainer, _extends({}, other, {
    ref,
    children: preProcessedButtons.map((button, index) => React90.cloneElement(button.component, {
      key: index
    }))
  }));
});
...
function GridToolbarQuickFilter(props) {
  ...
  const [searchValue, setSearchValue] = React91.useState(() => quickFilterFormatter(quickFilterValues ?? []));
  const prevQuickFilterValuesRef = React91.useRef(quickFilterValues);
  React91.useEffect(() => {
  ...
  const updateSearchValue = React91.useCallback((newSearchValue) => {
    const newQuickFilterValues = quickFilterParser(newSearchValue);
    prevQuickFilterValuesRef.current = newQuickFilterValues;
    apiRef.current.setQuickFilterValues(newQuickFilterValues);
  }, [apiRef, quickFilterParser]);
  const debouncedUpdateSearchValue = React91.useMemo(() => debounce(updateSearchValue, debounceMs), [updateSearchValue, debounceMs]);
  React91.useEffect(() => debouncedUpdateSearchValue.clear, [debouncedUpdateSearchValue]);
  const handleSearchValueChange = React91.useCallback((event) => {
    const newSearchValue = event.target.value;
    setSearchValue(newSearchValue);
    debouncedUpdateSearchValue(newSearchValue);
  }, [debouncedUpdateSearchValue]);
  const handleSearchReset = React91.useCallback(() => {
    setSearchValue("");
    updateSearchValue("");
  }, [updateSearchValue]);
...
var useGridStateInitialization = (apiRef) => {
  ...
  const forceUpdate = React114.useCallback(() => {
  }, []);
